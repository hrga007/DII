...
export function UploadPage() {
  ...
  function handleFileSelect(f: File) {
    ...
    setFile(f); setPhase('idle'); setResult(null); setErrorMsg('')
  }
  ...
  async function handleImport() {
    ...
    try {
      ...
      setErrorMsg(String(err)); setPhase('error')
  ...
  const dropStyle = (): React.CSSProperties => {
    if (phase === 'running') return { borderColor: 'var(--p-rg)', backgroundColor: 'var(--p-lt)' }
    if (dragging)            return { borderColor: 'var(--p)',    backgroundColor: 'var(--p-lt)', cursor: 'copy' }
    return {}
  }
  ...
  return (
    <div className="max-w-xl mx-auto">
      ...
      <div
        ...
        className="border-2 border-dashed rounded-2xl p-8 sm:p-12 text-center transition-colors border-gray-300"
        style={{ cursor: phase === 'running' ? 'default' : 'pointer', ...dropStyle() }}
        onMouseEnter={e => { if (phase !== 'running' && !dragging) (e.currentTarget as HTMLElement).style.borderColor = 'var(--p-rg)' }}
        onMouseLeave={e => { if (phase !== 'running' && !dragging) (e.currentTarget as HTMLElement).style.borderColor = '' }}
      >
        <input
          ref={inputRef}
          type="file"
        ...
        {file ? (
          <div>
            ...
            {phase === 'idle' && <p className="text-xs mt-2 p-tx">Klikni za promjenu datoteke</p>}
          </div>
        ) : (
          <div>
            ...
            <p className="text-xs text-gray-400 mt-3 bg-gray-100 inline-block px-3 py-1 rounded-full">.xlsx · .xls</p>
          </div>
        )}
      </div>
      ...
      {phase === 'running' && (
        <div className="mt-4 bg-white rounded-2xl border border-gray-200 p-4">
          <div className="flex items-center gap-2 mb-4">
            <div className="animate-spin h-4 w-4 border-2 spin-primary rounded-full shrink-0" />
            <span className="text-sm font-medium p-tx">{progress?.message}</span>
          </div>
          <div className="space-y-1.5">
            {STEPS.filter((s) => s.key !== 'done').map((s, i) => {
              ...
              return (
                <div key={s.key}
                  className={`flex items-center gap-2 text-xs ${done ? 'text-green-600' : !active ? 'text-gray-300' : ''}`}
                  style={active ? { color: 'var(--p-tx)', fontWeight: 500 } : {}}
                >
      ...
      {phase === 'error' && errorMsg && (
        <div className="mt-4 bg-red-50 border border-red-200 rounded-2xl px-4 py-3 text-sm text-red-700 flex items-start gap-2">
          <span className="text-base shrink-0">⚠️</span>{errorMsg}
        </div>
      )}
        ...
        <div className="mt-4 bg-green-50 border border-green-200 rounded-2xl p-4">
          ...
          <div className="flex gap-2">
            <button
              ...
              className="flex-1 bg-green-700 hover:bg-green-800 text-white py-3 rounded-xl text-sm font-semibold transition-colors"
            >
              Prikaži detalje
            </button>
            <button
      ...
      {file && phase === 'idle' && (
        <button
          ...
          className="btn-primary mt-4 w-full py-4 rounded-2xl font-semibold text-base"
        >
          Pokreni uvoz
        </button>
      )}
